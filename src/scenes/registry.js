...
import { initCombiScene, updateCombiScene, cleanupCombiScene } from './combi.js';
...
combi: {
init: initCombiScene,
update: updateCombiScene,
cleanup: cleanupCombiScene
}
};
...
throw new Error(`Unknown scene type: ${sceneType}`);
	...
	if (currentScene && currentSceneCleanup) {
		...
		try {
			// Stop animation loop first to prevent GPU access during cleanup
			currentSceneCleanup();

			// Wait for cleanup with delay to ensure GPU resources are released
